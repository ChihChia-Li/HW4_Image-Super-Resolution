...
class run:
    ...
    def test(self):
        ...
        print("\nTesting......\n")
        for n in range(14):
          folder = "/content/drive/MyDrive/DL_HW4/testing_lr_images/"
          if(n < 10):
            filename = os.path.join(folder + '0' + str(n) + '.png')
          elif(n >= 10):
            filename = os.path.join(folder + str(n) + '.png')
          print('Test file: ', filename)
          fullimg = cv2.imread(filename, 3)
          img = cv2.resize(fullimg, (fullimg.shape[1]*3, fullimg.shape[0]*3), interpolation=cv2.INTER_CUBIC)
          width = img.shape[0]
          height = img.shape[1]

          cropped = img[0:(width - (width % self.scale)), 0:(height - (height % self.scale)), :]
          ...
          with tf.compat.v1.Session(config=self.config) as sess:
              ...
              if(n < 10):
                saveimg_path = os.path.join("./images/" + '0' + str(n) + '.png')
              else:
                saveimg_path = os.path.join("./images/" + str(n) + '.png')
